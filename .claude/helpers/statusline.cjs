...
function getStatuslineData() {
  ...
  try {
    const raw = execSync("npx --yes @claude-flow/cli@latest hooks statusline --json 2>/dev/null", {
      encoding: "utf-8",
      timeout: 8000,
...
function generateStatusline() {
  ...
  // Line 1: DDD Domains
  const domainsColor =
    domainsCompleted >= 3 ? c.brightGreen : domainsCompleted > 0 ? c.yellow : c.red;
  let perfIndicator;
  if (hasHnsw && vectorCount > 0) {
    ...
    const pk =
      patternsLearned >= 1000 ? (patternsLearned / 1000).toFixed(1) + "k" : String(patternsLearned);
  ...
  const intellColor =
    intelligencePct >= 80 ? c.brightGreen : intelligencePct >= 40 ? c.brightYellow : c.dim;
